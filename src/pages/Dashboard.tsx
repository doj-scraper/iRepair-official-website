    ...
    const load = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from("orders")
          .select(`id, status, total_amount, created_at, stripe_session, order_items ( id, sku_id, quantity, unit_price, inventory ( part_name, image_url ) )`)
          .eq("user_id", user.id)
      ...
      <div className="container py-8 md:py-12">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <section aria-labelledby="user-summary" className="md:col-span-1">
            <h2 id="user-summary" className="sr-only">
              User summary
            </h2>
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              {user?.created_at && (
                <p className="mt-1 text-xs text-muted-foreground">
                  Account age: {formatDistanceToNow(new Date(user.created_at), { addSuffix: true })}
                </p>
              )}
            </div>
          </section>
